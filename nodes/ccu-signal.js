        ...
        sendCommand(config) {
            ...
            switch (config.channelType) {
                case 'SIGNAL_CHIME':
                    ...
                    payload = [config.volume / 100, config.repeat, 108_000, ...config.chime.split(',')];
                    console.log('payload', payload);
                    return this.ccu.setValue(config.iface, config.channel, 'SUBMIT', payload);
                case 'SIGNAL_LED':
                    payload = ['1', config.repeat, 108_000, ...config.led.split(',')];
                    return this.ccu.setValue(config.iface, config.channel, 'SUBMIT', payload);
            ...
            return new Promise((resolve, reject) => {
                ...
                switch (type) {
                    ...
                    case 'global': {
                        ...
                        this.context()[type].get(contextKey.key, contextKey.store, (error, response) => {
                            if (error) {
                                reject(error);
                            } else {
